...
function createRoom(e){
  if ($("#create-room-name").val() != "") {
    socket.emit("create", {name: $("#create-room-name").val(), pass: $("#create-room-pass").val()});
  }
  else {
...
function joinRoom(a){
  if(a == true){
    ...
    if(pass != ""){
      socket.emit("join", {name: $("#enter-room .modal-title").html(), pass: pass});
    }
  }
  else{
    socket.emit("join", {name:  a.currentTarget.id, pass: ""});
...
$(document).ready(function() {
  ...
  socket.emit("join", {name: "global", pass: ""});
  ...
  $("#create-room").submit(function(e) {
      var roominfo = {name: $("#room-name").val(), pass: $("#room-pass").val()};
...
jQuery.fn.extend({
  message: function(a, content, extension = null) {
    ...
    var data = {msg: document.getElementsByClassName("message-flow")[currentRoom].lastChild.outerHTML, room: rooms[currentRoom]};
